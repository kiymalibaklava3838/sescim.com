          ...
          <div className="text-center mb-10">
            <div className="inline-flex items-center gap-3 mb-6">
              <div className="font-display leading-none text-left">
                <div className="text-slate-900 font-black text-3xl tracking-wide uppercase">sescim</div>
                <div className="text-brand-red text-sm tracking-[0.3em] uppercase">.com</div>
              </div>
            </div>
            <h1 className="font-display font-black text-2xl uppercase text-slate-900 tracking-widest">Admin Girişi</h1>
            <p className="font-body text-slate-900/30 text-sm mt-2">Yönetim paneline erişmek için giriş yapın.</p>
          </div>
      ...
      <div className="bg-slate-50 border-b border-slate-200 py-10">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <div>
            ...
            <h1 className="font-display font-black text-4xl uppercase text-slate-900">ADMİN PANELİ</h1>
            <p className="font-body text-slate-900/30 text-sm mt-1">sescim.com — {user.email}</p>
          </div>
          <button onClick={handleLogout}
            className="flex items-center gap-2 text-slate-900/30 hover:text-brand-red font-display font-semibold text-xs tracking-widest uppercase transition-colors">
            ...
            return (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`flex items-center gap-2 px-5 py-4 font-display font-semibold text-xs tracking-widest uppercase border-b-2 transition-all duration-200 whitespace-nowrap relative flex-shrink-0 ${
                  activeTab === tab.id ? 'border-brand-red text-slate-900' : 'border-transparent text-slate-900/30 hover:text-slate-900/60'
                }`}>
                <Icon size={14} />
                {tab.label}
                {'badge' in tab && tab.badge ? (
                  <span className="w-4 h-4 bg-brand-red text-slate-900 text-[9px] font-black rounded-full flex items-center justify-center">
        ...
        {activeTab === 'urunler' && (
          <div className="grid lg:grid-cols-3 gap-12">
            <div className="lg:col-span-1">
              <h2 className="font-display font-bold text-xl uppercase tracking-wide text-slate-900 mb-6 red-line">Yeni Ürün Ekle</h2>
              <AdminAddProduct onAdded={() => setRefreshTrigger(prev => prev + 1)} />
            </div>
            <div className="lg:col-span-2">
              <h2 className="font-display font-bold text-xl uppercase tracking-wide text-slate-900 mb-6 red-line">Mevcut Ürünler</h2>
