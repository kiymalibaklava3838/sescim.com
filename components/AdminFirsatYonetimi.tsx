    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h2 className="font-display font-black text-2xl uppercase text-white">Flaş İndirimler</h2>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleSave} className="bg-[#141414] border border-white/10 p-6 space-y-4">
          <h3 className="font-display font-bold text-sm uppercase tracking-widest text-white">Yeni Flaş İndirim Ekle</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="font-display text-xs tracking-widest uppercase text-white/40 block mb-2">Ürün Seçin</label>
            ...
            <div>
              <label className="font-display text-xs tracking-widest uppercase text-white/40 block mb-2">İndirimli Fiyat (TL)</label>
            ...
            <div>
              <label className="font-display text-xs tracking-widest uppercase text-white/40 block mb-2">Başlangıç Tarihi</label>
              <input type="datetime-local" value={baslangic} onChange={e => setBaslangic(e.target.value)}
                className="w-full bg-[#0F0F0F] border border-white/10 text-white px-4 py-3 text-sm font-body focus:outline-none focus:border-brand-red/50" />
            </div>
            <div>
              <label className="font-display text-xs tracking-widest uppercase text-white/40 block mb-2">Bitiş Tarihi</label>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {firsatlar.map(firsat => {
            ...
            return (
              <div key={firsat.id} className={`bg-[#141414] border p-5 flex flex-col justify-between gap-4 ${
                firsat.aktif && !isGecmis ? 'border-brand-red/30' : 'border-white/5 opacity-70'
              }`}>
                <div>
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2 text-brand-red">
                      <Zap size={16} className={firsat.aktif && !isGecmis ? 'animate-pulse' : 'text-white/30'} />
                    ...
                    <div className="flex gap-2">
                      <span className={`font-display font-bold text-[10px] tracking-widest uppercase px-2 py-0.5 ${
                        !firsat.aktif ? 'bg-white/5 text-white/30 border border-white/10' :
                        isGecmis ? 'bg-red-500/10 text-red-400 border border-red-500/20' :
                        isBaslamadi ? 'bg-orange-500/10 text-orange-400 border border-orange-500/20' :
                  ...
                  <h4 className="font-bold text-white text-sm mb-2 line-clamp-1">{firsat.urun?.ad || 'Bilinmeyen Ürün'}</h4>
                  
                  <div className="flex items-end gap-3 mb-4">
                    <div className="text-2xl font-black font-display text-white">₺{firsat.indirimli_fiyat}</div>
                    <div className="text-sm text-white/30 line-through mb-1">₺{firsat.urun?.fiyat}</div>
                  </div>
                ...
                <div className="flex gap-2 mt-4 pt-4 border-t border-white/5">
                  <button onClick={() => toggleAktif(firsat.id, firsat.aktif)}
                    className={`flex-1 py-2 font-display text-[10px] tracking-widest uppercase border transition-all ${
                      firsat.aktif
                  ...
                  <button onClick={() => deleteFirsat(firsat.id)}
                    className="w-10 flex items-center justify-center border border-white/10 text-white/30 hover:border-red-500/30 hover:text-red-400 transition-all">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
      ...
      {!loading && firsatlar.length === 0 && (
        <div className="py-12 text-center text-white/20 font-body border border-white/5 bg-[#141414]">
